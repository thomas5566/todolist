...
from django.contrib.auth.decorators import login_required  # 只有登入者才能造訪網頁
...
def completedtodos(request):
    todos = Todo.objects.filter(
        user=request.user, datecompleted__isnull=False  # 事件完成設為False
    ).order_by(
        "-datecompleted"
    )  # filter 可以設定model要找的參數, model user = request user
...
def viewtodo(request, todo_pk):  # 丟url.py設定的todo_pk到這
    ...
    if request.method == "GET":
        form = TodoForm(instance=todo)  # grap data form TodoForm
